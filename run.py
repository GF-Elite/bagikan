...
{K}[{P}•{K}]{P}——————————————————————————————
{H}[{P}>{H}]{P} Au : Andika
{H}[{P}>{H}]{P} Ig : @Andika_12124123
{H}[{P}>{H}]{P} Fb : fb.com/kananda.widiantara.50
{K}[{P}•{K}]{P}——————————————————————————————
""")
...
os.system('clear')
print(" * Bot share facebook! Pastikan postingan tidak privat.\n")
	...
	def gasken(self,jumlah,komen):
		...
		for x in range(jumlah):
			...
			if "Anda Tidak Dapat Berkomentar Saat Ini" in kirim.text:
				os.system("rm -rf tumbal/coki.text")
				exit(" × Akun terkena limit harap ganti tumbal lagi !")
			elif "Akun Anda Dikunci" not in kirim.text:
				if(x==1):
					print(f" GF-Elite> Share berhasil {x} ")
				else:
					print(f" GF-Elite> Share berhasil {x} ")
			else:
				os.system("rm -rf tumbal/coki.text")
